...
@section('main')
    <div class="justify-center rounded flex bg-white dark:bg-slate-700 pb-10">
        <div class="justify-center bg-white dark:bg-slate-700  pb-10">
            <div class="rounded bg-slate-200 dark:bg-slate-400  py-6 px-6 mx-1 mt-5 mb-6  text-center text-4xl">Screen is Here</div>
            @foreach($screening->theater->seats as $seat)
                ...
                @else
                    <a href="{{ route('tickets.create',['seat' => $seat,'screening' => $screening]) }}" class="rounded bg-slate-200 dark:bg-slate-400 hover:bg-slate-400 dark:hover:bg-slate-200 lg:py-2 md:py-2 sm:py-1 lg:px-4 md:px-4 sm:px-3 mx-1 sm:mx-0.5 my-20"></a>
                @endif
            @endforeach
        </div>
    </div>
